...
def download_datasets():
    """Download only two fraud detection datasets to save space."""
    ...
    try:
        ...
        for file in os.listdir(kaggle_path_1):
            if file.endswith('.csv'):
                ...
                # Only keep a sample to save space (10,000 rows)
                if len(df1) > 10000:
                    # Ensure we keep some fraud cases by stratified sampling
                    df1 = df1.groupby('isFraud', group_keys=False).apply(
                        lambda x: x.sample(min(len(x), 5000 if x.name == 1 else 5000))
                    )
                    print(f"Reduced dataset 1 to {len(df1)} rows")
    ...
    # Dataset 2: Online Payment Fraud
    try:
        print("\nDownloading Dataset 2: Online Payment Fraud...")
        kaggle_path_2 = kagglehub.dataset_download("jainilcoder/online-payment-fraud-detection")
        print(f"Path to dataset files: {kaggle_path_2}")
        
        # Find the main CSV file (we know it's onlinefraud.csv from the previous code)
        source_file_2 = os.path.join(kaggle_path_2, 'onlinefraud.csv')
        if os.path.exists(source_file_2):
            # Read only a fraction of the file to save space
            df2 = pd.read_csv(source_file_2, nrows=10000)
            
            # Add dataset source column
            df2['dataset_source'] = 'online_payment'
            
            print(f"Dataset 2 shape: {df2.shape}")
            print(f"Dataset 2 fraud distribution: \n{df2['isFraud'].value_counts()}")
            
            # Save to data directory
            output_file_2 = os.path.join(data_dir, 'online_payment_fraud.csv')
            df2.to_csv(output_file_2, index=False)
            print(f"Saved Online Payment Fraud dataset to {output_file_2}")
...
def normalize_dataset_schema(dataframes):
    ...
    for i, df in enumerate(dataframes):
        ...
        if len(columns_to_keep) > 15:  # Using a smaller limit to save space
            print(f"Warning: Limiting to 15 most important columns for dataset {i}")
            columns_to_keep = columns_to_keep[:15]
...
def main():
    ...
    1. Download datasets (limited to save space)
    2. Normalize schemas
    3. Preprocess data
    4. Train model
    5. Save model
    """
    print("=== Fraud Detection System: Download and Training (Limited Dataset Version) ===")
