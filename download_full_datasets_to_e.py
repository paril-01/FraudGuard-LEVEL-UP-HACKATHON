...
def download_datasets():
    """Download all four fraud detection datasets to E: drive."""
    print("Starting dataset downloads...")
    
    # Create data directory on drive E
    data_dir = 'E:/fraud_detection_data'
    ...
    try:
        ...
        kaggle_path_1 = kagglehub.dataset_download("priyanshugautam1214/upi-transactions-23-24", force=True)
        ...
        for file in os.listdir(kaggle_path_1):
            if file.endswith('.csv'):
                ...
                # Copy to E drive
                dest_file_1 = os.path.join(data_dir, 'upi_transactions.csv')
                shutil.copy(source_file_1, dest_file_1)
                print(f"Copied dataset to {dest_file_1}")
                
                # Read the data
                df1 = pd.read_csv(dest_file_1)
                ...
                if 'is_fraud' in df1.columns:
                    ...
                    if 'amount' in df1.columns or 'transaction_amount' in df1.columns or 'Transaction Amount' in df1.columns:
                        amount_col = 'amount' if 'amount' in df1.columns else (
                            'transaction_amount' if 'transaction_amount' in df1.columns else 'Transaction Amount'
                        )
                ...
                output_file_1 = os.path.join(data_dir, 'upi_transactions_processed.csv')
                df1.to_csv(output_file_1, index=False)
                
                print(f"Dataset 1 shape: {df1.shape}")
                print(f"Dataset 1 fraud distribution: \n{df1['isFraud'].value_counts()}")
                print(f"Download and processing time: {time.time() - start_time:.2f} seconds")
    ...
    try:
        ...
        start_time = time.time()
        kaggle_path_2 = kagglehub.dataset_download("kartik2112/fraud-detection", force=True)
        ...
        for file in os.listdir(kaggle_path_2):
            if file.endswith('.csv'):
                ...
                # Copy to E drive
                dest_file_2 = os.path.join(data_dir, 'credit_card_fraud.csv')
                shutil.copy(source_file_2, dest_file_2)
                print(f"Copied dataset to {dest_file_2}")
                
                # Read the data
                df2 = pd.read_csv(dest_file_2)
                ...
                # Save processed version
                output_file_2 = os.path.join(data_dir, 'credit_card_fraud_processed.csv')
    ...
    try:
        ...
        start_time = time.time()
        kaggle_path_3 = kagglehub.dataset_download("nelgiriyewithana/credit-card-fraud-detection-dataset-2023", force=True)
        ...
        for file in os.listdir(kaggle_path_3):
            if file.endswith('.csv'):
                ...
                # Copy to E drive
                dest_file_3 = os.path.join(data_dir, 'cc_fraud_2023.csv')
                shutil.copy(source_file_3, dest_file_3)
                print(f"Copied dataset to {dest_file_3}")
                
                # Read the data
                df3 = pd.read_csv(dest_file_3)
                ...
                # Save processed version
                output_file_3 = os.path.join(data_dir, 'cc_fraud_2023_processed.csv')
    ...
    try:
        ...
        start_time = time.time()
        kaggle_path_4 = kagglehub.dataset_download("jainilcoder/online-payment-fraud-detection", force=True)
        print(f"Path to dataset files: {kaggle_path_4}")
        
        # Find the onlinefraud.csv file
        source_file_4 = os.path.join(kaggle_path_4, 'onlinefraud.csv')
        if os.path.exists(source_file_4):
            # Copy to E drive
            dest_file_4 = os.path.join(data_dir, 'online_payment_fraud.csv')
            shutil.copy(source_file_4, dest_file_4)
            print(f"Copied dataset to {dest_file_4}")
            
            # Read the data (in chunks if it's too large)
            chunk_size = 1000000  # 1 million rows per chunk
            chunks = []
            
            for chunk in pd.read_csv(dest_file_4, chunksize=chunk_size):
                chunk['dataset_source'] = 'online_payment'
                chunks.append(chunk)
            
            df4 = pd.concat(chunks, ignore_index=True)
            
            # Save processed version (not saving to avoid duplicate storage of large file)
            # output_file_4 = os.path.join(data_dir, 'online_payment_fraud_processed.csv')
...
def normalize_dataset_schema(dataframes):
    ...
    for i, df in enumerate(dataframes):
        ...
        if 'amount' not in df.columns:
            amount_alternatives = ['amt', 'transaction_amount', 'tx_amount', 'Amount', 'Transaction Amount']
            found = False
    ...
    for col in all_columns:
        if col.lower() in ['type', 'transaction_type', 'tx_type', 'transaction category']:
            for df in dataframes:
    ...
    # For large datasets, process in chunks to save memory
    if any(len(df) > 1000000 for df in normalized_dfs):
        print("Processing large datasets in chunks...")
        combined_df_path = "E:/fraud_detection_data/combined_normalized.csv"
        
        # Create a header row first
        sample_df = pd.concat([df.head(1) for df in normalized_dfs], ignore_index=True)
        sample_df.to_csv(combined_df_path, index=False)
        
        # Append each dataframe chunk by chunk
        for i, df in enumerate(normalized_dfs):
            chunk_size = 500000  # Process 500k rows at a time
            for start in range(0, len(df), chunk_size):
                end = min(start + chunk_size, len(df))
                chunk = df.iloc[start:end].copy()
                chunk.to_csv(combined_df_path, mode='a', header=False, index=False)
                print(f"Processed {end}/{len(df)} rows from dataset {i}")
        
        # Load the combined data back for further processing
        combined_df = pd.read_csv(combined_df_path)
    else:
        # If datasets are small enough, just concat them in memory
...
def preprocess_data(df, test_size=0.3, random_state=42):
    ...
    # For very large datasets, use a smaller subset for training
    if len(df) > 1000000:
        print(f"Dataset too large ({len(df)} rows). Using stratified sample of 1,000,000 rows for training.")
        X_sample, _, y_sample, _ = train_test_split(
            X, y, train_size=1000000, random_state=random_state, stratify=y
        )
...
def train_model(X_train, y_train, X_test, y_test):
    ...
    model = xgb.XGBClassifier(
        ...
        learning_rate=0.1,
        n_estimators=200,  # Increased for better performance on full dataset
        max_depth=6,
        subsample=0.8,
...
def save_model(model, preprocessor, feature_names, data_dir):
    """
    ...
    try:
        ...
        # Save model
        model_path = os.path.join(model_dir, 'xgboost_full.pkl')
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
        
        # Save preprocessor
        preprocessor_path = os.path.join(model_dir, 'xgboost_preprocessor_full.pkl')
        with open(preprocessor_path, 'wb') as f:
            pickle.dump(preprocessor, f)
        
        # Save feature names
        feature_names_path = os.path.join(model_dir, 'xgboost_feature_names_full.pkl')
        ...
        print("Continuing without saving model files...")
        return False

def main():
    """
    Main function to execute the workflow:
    1. Download datasets to drive E
    2. Normalize schemas
    3. Preprocess data
    4. Train model
    5. Save model to drive E
    """
    print("=== Fraud Detection System: Full Dataset Training on Drive E ===")
    start_time = time.time()
    
    # Step 1: Download datasets to Drive E
    print("\nStep 1: Downloading datasets to Drive E...")
    datasets, data_dir = download_datasets()
    if not datasets:
        print("Error: No datasets were downloaded. Exiting.")
        return
    
    # Step 2: Normalize schemas
    print("\nStep 2: Normalizing dataset schemas...")
    combined_df = normalize_dataset_schema(datasets)
    
    # Step 3: Preprocess data with 70-30 train-test split
    print("\nStep 3: Preprocessing data with 70-30 train-test split...")
    X_train, X_test, y_train, y_test, preprocessor, feature_names = preprocess_data(
        combined_df, test_size=0.3, random_state=42
    )
    
    # Step 4: Train model
    print("\nStep 4: Training model with full dataset...")
    model = train_model(X_train, y_train, X_test, y_test)
    
    # Step 5: Save model to Drive E
    print("\nStep 5: Saving model to Drive E...")
    save_model(model, preprocessor, feature_names, data_dir)
    ...
    print(f"\n=== Training process completed successfully in {hours}h {minutes}m {seconds}s ===")
